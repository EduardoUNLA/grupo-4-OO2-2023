...
public class EventoService implements IEventoService {
	...
	@Override
	public List<Evento> getEventosSensorTemperatura() {
		List<Evento> eventos = eventoRepository.findAll();
		List<Evento> eventosSensorTemperatura = new ArrayList<>();
		for(int i=0; i<eventos.size();i++) {
			if(eventos.get(i).getDispositivo() instanceof SensorTemperatura) {
				eventosSensorTemperatura.add(eventos.get(i));
			}
		}
		return eventosSensorTemperatura;
	}
